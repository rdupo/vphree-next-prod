...
const getNllHistory = (wallet) => {
  ...
  const [nllTxnHistory, setNllTxnHistory] = useState([]);
  ...
  const fetchTransactionHistory = async () => {
    ...
    const contract = new ethers.Contract(nllAddy, nllAbi, provider);
    ...
    const marketplaceEvents = [...Offer, ...Withdraw, ...Sold, ...Bought];
    ...
    if (Offer.length === 0 && Withdraw.length === 0 && Sold.length === 0 && Bought.length === 0) {
      ...
      setNllTxnHistory([{eventType:'none'}]);
      return;
    }
    ...
    const refilteredEvents = combinedEvents.filter(event => {
      return !nllTxnHistory.some(prevEvent => prevEvent.timestamp === event.timestamp);
    });
    ...
    Promise.all(formattedEvents).then((results) => {
      if (results.length > 0) {
        setNllTxnHistory(results);
        //console.log('txn', nllTxnHistory);
      }
    });
  };
  ...
  return { nllTxnHistory };
};

export default getNllHistory;